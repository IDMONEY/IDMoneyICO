...
contract NEToken is StandardToken {
    ...
    isFundraising
    {
        ...
        require(block.number > fundingStartBlock); // Check that the block number is bigger than the fundingStartBlock and
        require(msg.value > 0);  // that the amount of Ether sent was bigger than 0

        // First we check the ETH cap, as it's easier to calculate, return
        // the contribution if the cap has been reached already
        require(safeAdd(totalReceivedEth,msg.value) > ETH_RECEIVED_CAP);
        ...
        require(msg.value > ETH_RECEIVED_MIN);

        uint256 ethreceive = msg.value;
        uint256 tokreceive = getCurrentTokenPrice() * msg.value;
        ...
        require( (totalSupply + tokreceive) <= TOKEN_CREATION_CAP);
        
        uint256 netVal = balances[msg.sender];
        uint256 netEth = ethBalances[msg.sender];

        
        balances[msg.sender] = safeAdd(netVal,tokreceive);
        ethBalances[msg.sender] = safeAdd(netEth,ethreceive);

        totalReceivedEth = safeAdd(totalReceivedEth,ethreceive);
        totalSupply = safeAdd(totalSupply,tokreceive);

        // Log the creation of this tokens
        LogCreateNET(msg.sender, netVal);
